...
public class DrawingTool_Brush : MonoBehaviour
{
    ...
    {
        // 왼손 그림
        if (leftObj != null && leftObj.CompareTag(brushTag) && IsTriggerPressed(leftDevice))
        {
            if (!isLeftDrawing)
            {
        ...
        // 오른손 그림
        if (rightObj != null && rightObj.CompareTag(brushTag) && IsTriggerPressed(rightDevice))
        {
            if (!isRightDrawing)
            {
