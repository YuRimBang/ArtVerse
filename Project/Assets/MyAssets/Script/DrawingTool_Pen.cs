...
public class DrawingToolPen : MonoBehaviour
{
    ...
    private float drawingSpeed = 0.002f;   // 그림 그릴 속도
    ...
    private string pencilTag = "Pencil";
    ...
    {
        // 왼손 그림
        if (leftObj != null && leftObj.CompareTag(pencilTag) && IsTriggerPressed(leftDevice))
        {
            if (!isLeftDrawing)
            {
        ...
        // 오른손 그림
        if (rightObj != null && rightObj.CompareTag(pencilTag) && IsTriggerPressed(rightDevice))
        {
            if (!isRightDrawing)
            {
    ...
    public void SetLeftObj(GameObject pencil)
    {
        leftObj = pencil;
    }

    public void SetRightObj(GameObject pencil)
    {
        rightObj = pencil;
    }
